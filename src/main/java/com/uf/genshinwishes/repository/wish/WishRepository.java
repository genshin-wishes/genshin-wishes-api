    ...
    @Query(value = """
        ...
        where w.index = (
            select min(w2.index) from wishes w2
                where (w2.import_date is not null and w2.import_date >= :importArchiveDate or w2.time >= :archiveDate) and w2.user_id = :userId and w2.gacha_type = :gachaType
        ) and w.user_id = :userId and w.gacha_type = :gachaType
        """, nativeQuery = true)
    Optional<Wish> findFirstNonArchived(Long userId, Integer gachaType, Instant importArchiveDate, LocalDateTime archiveDate);
